...
const onEvent = (conn, uri) => {
  const sigint_cb = () => {
    mongoose.disconnect(() => {
      console.log(chalk.blue(uri, '-disconnected on app termination by SIGINT'));
      process.exit(0);
    });
  };
  ...
  conn.on('error', (err) => {
    console.error(chalk.red(uri, err, 'readyState:' + conn.readyState));
  });

  conn.on('connected', () => {
    console.info(chalk.blue(uri, '-connected'));
  });

  conn.on('open', () => {
    // console.info(chalk.blue(uri, '-connection open'));
  });

  conn.on('reconnected', () => {
    console.log(chalk.blue(uri, '-connection reconnected'));
  });

  conn.on('disconnected', () => {
    console.log(chalk.blue(uri, '-connection disconnected'));
