...
class CommonMethods {
  ...
  // delete one doc. First find doc, then delete doc. This is because doc.deleteOne() activate post middleware. this.model.findOneAndRemove() will not activate middleware.
  deleteOne(moQuery) {
  ...
  countDocs(moQuery) {
    return this.model.countDocuments(moQuery).then(count => +count || 0); // + is converting string into number
  }
